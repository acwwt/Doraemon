from LLM import Deepseek_LLM, InterLM_LLM, Moonshot_LLM, Step_LLM
...
def save_data(data):
    ...
    with open(f"./generation_dataset/{filename}.jsonl", mode='a', encoding='utf-8') as f:
...
def read_dialogue(path):
    ...
    filename = path.split('/')[-1].split('.')[0]
    res = []
    with open(f"./dataset/{filename}.jsonl", mode='r', encoding='utf-8') as file:
...
def save_dataset(path, data):
    filename = path.split('/')[-1].split('.')[0]
    with open(f"./dataset/{filename}.json", mode='w', encoding='utf-8') as f:
        f.write(json.dumps(data, ensure_ascii=False))
